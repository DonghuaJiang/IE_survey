...
p = min(max(2,p),N-1);
E = dFRFT(N,p);
y(shft,1)=E*(exp(-j*pi/2*a*([0:N-2 N-1+even])).'.*(E'*f(shft)));% more efficient to compute
...
for k = 1:p/2,
    d_p = conv(d2,d_p);% can be computed by p successive convolutions of the vector
    st([N-k+1:N,1:k+1]) = d_p; % symmetric
    st(1) = 0;% the diagonal elements correspond to the constant term c0 in the polynomial
    temp = [1:k;1:k];
    temp=temp(:)'./[1:2*k];
    s = s + (-1)^(k-1)*prod(temp)*2*st;   %why *2 ?     because of p/2 ?
end;
